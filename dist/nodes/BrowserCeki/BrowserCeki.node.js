...
var CekiClient = class {
  ...
  async disconnect() {
    ...
    await new Promise((r) => {
      const onAbort = () => r();
      const abortTimer = AbortSignal.timeout(5e3);
      abortTimer.addEventListener("abort", onAbort, { once: true });
      ws.onclose = () => {
        this._connected = false;
        abortTimer.removeEventListener("abort", onAbort);
        r();
      };
      try {
        ws.close();
...
// nodes/BrowserCeki/BrowserCeki.node.ts
async function waitForSelector(node, browser, selector, timeoutMs, intervalMs = 500) {
...
var BrowserCeki = class {
  ...
  async execute() {
    ...
    const resolveSid = async (i, client) => {
      ...
      if (!list.length) throw new import_n8n_workflow.NodeOperationError(node, "No browsers found by filters");
        ...
        try {
          ...
          if (op === "rent") {
            try {
              ...
              throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Rent failed" });
            } finally {
          ...
          if (op === "search") {
            try {
              ...
              throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Search failed" });
            } finally {
              await client.disconnect();
            }
            continue;
          ...
          if (op === "my_sessions") {
            try {
              ...
              if (!resp.ok) throw new import_n8n_workflow.NodeApiError(node, void 0, { itemIndex: i, message: `HTTP ${resp.status}`, httpCode: `${resp.status}` });
              const body = await resp.json();
              const sessions = body.data ?? [];
              out.push({ json: { sessions, count: sessions.length }, pairedItem: { item: i } });
            } catch (e) {
              throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "My sessions failed" });
            }
            continue;
          }
          if (op === "stop_session") {
            ...
            try {
              await new Promise((resolve, reject) => {
                ...
                t.addEventListener("abort", () => {
                  ...
                  reject(new import_n8n_workflow.NodeApiError(node, void 0, { itemIndex: i, message: "Stop session timed out" }));
                }, { once: true });
                ws.onopen = () => {
                ...
                ws.onerror = () => {
                  reject(new import_n8n_workflow.NodeApiError(node, void 0, { itemIndex: i, message: "WebSocket connection failed" }));
                };
              });
              out.push({ json: { stopped: true, session_id: sessionId2 }, pairedItem: { item: i } });
            } catch (e) {
              throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Stop session failed" });
            }
            continue;
          }
          if (op === "full") {
            ...
            if (demoMode) {
              ...
              try {
                ...
                throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Full: resolve schedule failed" });
              }
            }
            const mode = this.getNodeParameter("mode", i);
            ...
            try {
              if (demoMode) {
                ...
                try {
                  ...
                  throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Full op failed" });
                }
              }
              out.push({
                json: {
          ...
          if (op === "captchaScrape") {
            ...
            try {
              ...
              if (!list.length) throw new import_n8n_workflow.NodeOperationError(node, "No browsers in geo " + (geo || "*"), { itemIndex: i });
              ...
              throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Captcha-protected scrape failed" });
            } finally {
              if (browser) {
                try {
                  await browser.close();
          ...
          if (op === "screenshotGeo") {
            ...
            try {
              ...
              throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Screenshot in geo " + geo + " failed" });
            } finally {
              if (browser) {
                try {
                  await browser.close();
            ...
            try {
              switch (pop) {
                ...
                case "poll":
                  ...
                  break;
                default:
                  throw new import_n8n_workflow.NodeOperationError(node, "Unknown contract operation: " + pop, { itemIndex: i });
              }
              out.push({
                json: result,
                pairedItem: { item: i }
              ...
              throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Contract " + pop + " failed" });
            }
            continue;
          }
          ...
          try {
            ...
            throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: `Resume session "${sessionId}" failed. Session may have expired or still be in grace.` });
          }
          ...
          switch (op) {
            case "navigate": {
              try {
                ...
                throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Navigate failed" });
              }
              break;
            }
            case "click": {
              try {
                ...
                throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Click failed" });
              }
              break;
            }
            case "type": {
              try {
                ...
                throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Type failed" });
              }
              break;
            }
            case "scroll": {
              try {
                ...
                throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Scroll failed" });
              }
              break;
            }
            case "screenshot": {
              try {
                ...
                throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Screenshot failed" });
              }
              break;
            }
            case "snapshot": {
              try {
                ...
                throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Snapshot failed" });
              }
              break;
            }
            case "wait": {
            ...
            case "upload": {
              try {
                ...
                if (!bin) throw new import_n8n_workflow.NodeOperationError(node, `Binary property "${bpn}" not found on input`, { itemIndex: i });
                ...
                throw new import_n8n_workflow.NodeApiError(node, e, { itemIndex: i, message: "Upload failed" });
              }
              break;
            }
            case "close": {
