...
export class BrowserCeki implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const resolveSid = async (i: number, client: CekiClient) => {
			...
			const filters: Record<string, unknown> = { geo: geo || undefined, max_price_per_min: maxPrice };
			...
			try {
				...
				if (op === 'rent') {
					try {
						...
						throw new NodeApiError(this.getNode(), e as any, { message: 'Rent failed' });
					} finally {
						await client.disconnect().catch(() => { /* ignore — best-effort cleanup */ });
					}
					continue;
				}
				...
				if (op === 'search') {
					try {
						...
						if (e instanceof NodeOperationError) throw e;
						throw new NodeApiError(this.getNode(), e as any, { message: 'Search failed' });
					} finally {
						await client.disconnect().catch(() => { /* ignore — best-effort cleanup */ });
					}
					continue;
				...
				if (op === 'full') {
					...
					if (demoMode) {
						...
						try {
							...
							if (e instanceof NodeOperationError) throw e;
							throw new NodeApiError(this.getNode(), e as any, { message: 'Full: resolve schedule failed' });
						}
					}
					const mode = this.getNodeParameter('mode', i) as 'main' | 'incognito';
					...
					try {
						...
						await client.close().catch(() => { /* ignore — best-effort cleanup */ });
					}
					continue;
				}

				// ── Captcha-protected scrape ─────────────────────────
				if (op === 'captchaScrape') {
					const url = this.getNodeParameter('url', i) as string;
					const geo = this.getNodeParameter('geo', i) as string;
					const maxPrice = this.getNodeParameter('maxPrice', i) as number;
					const waitSelectorParam = (this.getNodeParameter('captchaWaitSelector', i) as string) || '';
					...
					try {
						...
						if (e instanceof NodeOperationError) throw e;
						throw new NodeApiError(this.getNode(), e as any, { message: 'Captcha-protected scrape failed' });
					} finally {
						if (browser) { try { await browser.close(); } catch { /* ignore — best-effort close */ } }
						await client.close().catch(() => { /* ignore — best-effort cleanup */ });
					}
					continue;
				}
				...
				if (op === 'screenshotGeo') {
					...
					try {
						...
						if (e instanceof NodeOperationError) throw e;
						throw new NodeApiError(this.getNode(), e as any, { message: 'Screenshot in geo failed' });
					} finally {
						if (browser) { try { await browser.close(); } catch { /* ignore — best-effort close */ } }
						await client.close().catch(() => { /* ignore — best-effort cleanup */ });
					}
				...
				// ── Remaining operations: session-based ──────────────
				...
				switch (op) {
					case 'navigate': {
						try {
							...
							throw new NodeApiError(this.getNode(), e as any, { message: 'Navigate failed' });
						}
						break;
					}
					case 'click': {
						try {
							...
							throw new NodeApiError(this.getNode(), e as any, { message: 'Click failed' });
						}
						break;
					}
					case 'type': {
						try {
							...
							throw new NodeApiError(this.getNode(), e as any, { message: 'Type failed' });
						}
						break;
					}
					case 'scroll': {
						try {
							...
							throw new NodeApiError(this.getNode(), e as any, { message: 'Scroll failed' });
						}
						break;
					}
					case 'screenshot': {
						try {
							...
							throw new NodeApiError(this.getNode(), e as any, { message: 'Screenshot failed' });
						}
						break;
					}
					case 'snapshot': {
						try {
							...
							throw new NodeApiError(this.getNode(), e as any, { message: 'Snapshot failed' });
						}
						break;
					}
					case 'wait': {
					...
					case 'waitForSelector': {
						...
						try {
							...
							throw new NodeApiError(this.getNode(), e as any, { message: `waitForSelector("${selector}") timed out` });
						}
						break;
					}
					case 'upload': {
						try {
							...
							if (e instanceof NodeOperationError) throw e;
							throw new NodeApiError(this.getNode(), e as any, { message: 'Upload failed' });
						}
						break;
					}
					case 'close': {
						try {
							await browser.close().catch(() => { /* ignore — best-effort close */ });
						} catch {
							/* ignore — CDP close errors */
						}
						// Always send relay stop (force-end session)
